...
- [ ] Explain the difference between `Relational Databases` and `SQL`.
      Relational Database - is a set of formally described tables from which data can be accessed or reassembled.
      SQL - is the programming (API) used to work with the database.
- [ ] Why do tables need a `primary key`?
      Primary key - a table column that serves a special purpose. Each database table needs a primary key because it ensures row level accessibility. Each table has one and only one primary key, which can consist of one or many columns.
